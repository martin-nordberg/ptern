...
pub fn array_replace_fixed_repetition_test() {
  // Pattern matches 3 consecutive 4-digit groups with no spaces.
  let assert Ok(p) =
    ptern.compile("!replacements-ignore-matching = true\n(%Digit * 4 as yr) * 3")
  ptern.replace_first_in(
    p,
...
pub fn capture_in_repetition_test() {
  // Captures inside repetitions are allowed; the pattern compiles and matches.
  let assert Ok(p) = ptern.compile("('a' as x) * 3")
  ptern.matches_all_of(p, "aaa") |> should.be_true
}

pub fn capture_in_bounded_repetition_test() {
  let assert Ok(p) = ptern.compile("('a' as val) * 1..5")
  ptern.matches_all_of(p, "aaa") |> should.be_true
}

pub fn capture_in_optional_repetition_test() {
  let assert Ok(p) = ptern.compile("('a' as opt) * 0..1")
  ptern.matches_all_of(p, "a") |> should.be_true
}

pub fn capture_in_unbounded_repetition_test() {
  let assert Ok(p) = ptern.compile("(%Digit as d) * 1..?")
  ptern.matches_all_of(p, "123") |> should.be_true
}
...
pub fn duplicate_capture_test() {
  // Duplicate capture names are allowed; same replacement value applies to all occurrences.
  let assert Ok(p) = ptern.compile("'a' as x '-' 'b' as x")
  ptern.matches_all_of(p, "a-b") |> should.be_true
}

pub fn duplicate_capture_three_uses_test() {
  let assert Ok(p) = ptern.compile("'a' as v '-' 'b' as v '-' 'c' as v")
  ptern.matches_all_of(p, "a-b-c") |> should.be_true
}
